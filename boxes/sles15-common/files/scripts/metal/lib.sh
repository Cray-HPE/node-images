...
function write_can_route {
    ...
    local canif=$(craysys metadata get can-if)
    [ -z "${canif}" ] || [ -z "${canif}" ] || echo "default ${cangw} - bond0.can0" >>/etc/sysconfig/network/ifroute-bond0.can0 && wicked ifreload all || systemctl restart wickedd && sleep 3
}
